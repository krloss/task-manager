<template>
	<v-navigation-drawer fixed :clipped="$vuetify.breakpoint.mdAndUp" app v-model="superCtrl.drawer">
		<v-list dense> <template v-for="item in items">
			<v-layout row v-if="item.heading" align-center :key="item.heading">
				<v-flex xs6> <v-subheader v-if="item.heading"> {{ item.heading }} </v-subheader> </v-flex>
			...
			<v-list-group v-else-if="item.children" v-model="item.model" :key="item.text"
				...
				<v-list-tile slot="activator"> <v-list-tile-content>
